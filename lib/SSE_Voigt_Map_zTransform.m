function sse = SSE_Voigt_Map_zTransform(data,params,elasticSetting,fluidSetting)
...
%     z_model = alphaInit + 1i.*omega;
    if n_terms > 0
%         U_model = @(c,z) (c(1)+sum( (repmat(ones(size(z)),n_terms,1).*c(3:2:end).*(c(4:2:end)./dt))./((c(4:2:end)./dt).*(repmat(1-z.^-1,n_terms,1))+1) .* repmat(1-z.^-1,n_terms,1) , 1));
        U_storage = @(c,omega) c(1) + sum( (c(3:2:end).*repmat(ones(size(omega)),n_terms,1).*(((1+(c(4:2:end)./dt).*alphaInit).^2)./((c(4:2:end)./dt).^2)))./(((1+(c(4:2:end)./dt).*alphaInit).*repmat(ones(size(omega)),n_terms,1)).*((((1+(c(4:2:end)./dt).*alphaInit).^2)./((c(4:2:end)./dt).^2)).*repmat(ones(size(omega)),n_terms,1) + repmat(omega.^2,n_terms,1))), 1);
        U_loss = @(c,omega) sum( repmat(omega,n_terms,1).*(c(3:2:end).*(c(4:2:end)./dt).*repmat(ones(size(omega)),n_terms,1).*(((1+(c(4:2:end)./dt).*alphaInit).^2)./((c(4:2:end)./dt).^2)))./((((1+(c(4:2:end)./dt).*alphaInit).^2).*repmat(ones(size(omega)),n_terms,1)).*((((1+(c(4:2:end)./dt).*alphaInit).^2)./((c(4:2:end)./dt).^2)).*repmat(ones(size(omega)),n_terms,1) + repmat(omega.^2,n_terms,1))), 1);
    else
%         U_model = @(c,z) c(1).*ones(size(omega));
        U_storage = @(c,omega) c(1).*ones(size(omega));
        U_loss = @(c,omega) zeros(size(omega));
    end
